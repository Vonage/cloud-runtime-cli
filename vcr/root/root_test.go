...
// TestNewCmdRoot_proseMentionsTheTopLevelLogsCommand pins review finding 8: the
// generated docs/vcr.md advertises `vcr logs` under SEE ALSO, but root's
// hand-written Long/Example only ever showed `vcr instance log`, so the new
// entry point was undiscoverable from `vcr --help`.
func TestNewCmdRoot_proseMentionsTheTopLevelLogsCommand(t *testing.T) {
	ios, _, _, _ := iostreams.Test()
	f := testutil.DefaultFactoryMock(t, ios, nil, nil, nil, nil, nil, nil)
	...
	require.Contains(t, cmd.Long, "vcr logs", "CORE WORKFLOW must list the top-level logs command")
	require.Contains(t, cmd.Example, "$ vcr logs", "the examples must show a runnable vcr logs invocation")
}
